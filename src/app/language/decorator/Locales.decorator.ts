  ...
  return function(target) {
    if (shouldMark && !cdr) {
      console.warn(
        `${
          target.name
        } is using @Locale, asks for detectChanges, but does not inform ChageDetectionRef on the cdr parameter.`
      );
    }
    ...
    target.prototype.getLocales = function() {
      if (!this[store]) {
        console.warn(
          `${
            target.name
          } is using @Locale but does not have the property ${store} as a Store from @ngrx/Store.`
        );
      }

      this.store
        .pipe(
          select(getComponentLocale(module, name)),
